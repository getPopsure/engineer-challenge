 ...
 return (
   <div>
     ...
     {Object.keys(query).map(filter => (
       <button key={filter} className="mx-3 bg-gray-200 p-2 rounded">
         {filter}: {query[filter]}
         <span className="rounded-full bg-gray-500 text-white ml-2 inline-block px-1.5" onClick={() => handleRemoveFilter(filter)}>X</span>
       </button>
     ))}
   </div>
  );
